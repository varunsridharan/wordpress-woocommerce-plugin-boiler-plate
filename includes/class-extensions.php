...
class WooCommerce_Plugin_Boiler_Plate_Addons {
	...
	public function deactivate_plugin(){
		...
		if($status === 'invalidcode'){
			wp_send_json_error(array('msg' => '<span class="wc_pbp_ajax_error">'.__('Unable to process you request. please try again later',PLUGIN_TXT).'</span>'));
		} else if($status === 'verifyfailed'){
			wp_send_json_error(array('msg' => '<span class="wc_pbp_ajax_ajaxerror">'.__('Unable To De-Activate Addon. Please Try Again Later',PLUGIN_TXT).'</span>'));		
		} else if($status === true){
			wp_send_json_success(array('msg' => '<span class="wc_pbp_ajax_success">'.__('Addon De-Activated',PLUGIN_TXT).'</span>'));		
		} else if($status === 'alreadyactive'){
			wp_send_json_success(array('msg' => '<span class="wc_pbp_ajax_success">'.__('Addon Already De-Activated',PLUGIN_TXT).'</span>'));		
		} else {
			wp_send_json_error(array('msg' => '<span class="wc_pbp_ajax_error">'.__('Unable To De-Activate Addon. Please Try Again Later',PLUGIN_TXT).'</span>'));		
		}
		wp_die();  
	}
	...
	public function activate_plugin(){
		...
		if($status === 'invalidcode'){
			...
			wp_send_json_error(array('msg' => '<span class="wc_pbp_ajax_ajaxerror">'.__('Unable To Activate Addon. Please Try Again Later',PLUGIN_TXT).'</span>'));		
		} else if($status === true){
			wp_send_json_success(array('msg' => '<span class="wc_pbp_ajax_success">'.__('Addon Activated',PLUGIN_TXT).'</span>'));		
		} else if($status === 'alreadyactive'){
			wp_send_json_success(array('msg' => '<span class="wc_pbp_ajax_success">'.__('Addon Already Activated',PLUGIN_TXT).'</span>'));		
		} else {
			wp_send_json_error(array('msg' => '<span class="wc_pbp_ajax_error">'.__('Unable To Activate Addon. Please Try Again Later',PLUGIN_TXT).'</span>'));		
		}
		wp_die();  
	}
	
	public function addon_actions($action = 'activate'){
		if(! isset($_REQUEST['wc_pbp_security_code'])){ return 'invalidcode'; }
		$nonce_action = 'wc_pbp_'.$action.'_addon';
		$verify = wp_verify_nonce( $_REQUEST['wc_pbp_security_code'], $nonce_action );
		if(!$verify){return 'verifyfailed';}
		$function_call = 'wc_pbp_'.$action.'_addon';
	...
	public function search_and_get_addons(){
		$search_dirs = apply_filters('wc_pbp_addons_dir',array()); 
		$addons_others = array();
		$internal_addons = $this->get_plugins(PLUGIN_ADDON);
	...
	public function generate_view(){
		...
		include(PLUGIN_ADMIN.'views/addons-header.php');
		foreach($this->plugins_data as $addon_slug => $data){
			$wc_pbp_plugin_data = $data; 
			$wc_pbp_plugin_slug = $addon_slug;
			$required_plugins = $this->extract_required_plugins($wc_pbp_plugin_data);
			include(PLUGIN_ADMIN.'views/addons-single.php');
			unset($wc_pbp_plugin_data);
		}
		include(PLUGIN_ADMIN.'views/addons-footer.php');
	}
	...
	public function extract_required_plugins($wc_pbp_plugin_data){ 
		$plugins = $wc_pbp_plugin_data['rplugins'];
	...
	public function get_addon_action_link($plugin_slug,$type="active"){
		$is_active = wc_pbp_check_active_addon($plugin_slug);
		$action = 'wc_pbp_activate_addon';
		if($type == 'deactivate'){ $action = 'wc_pbp_deactivate_addon';} 
		$url = admin_url('admin-ajax.php?action='.$action.'&addon_slug='.$plugin_slug);
		$url = wp_nonce_url($url,$action, 'wc_pbp_security_code');
		return $url;
	}
	...
	public function get_addon_action_button($plugin_slug,$required_plugins){
		...
		$is_active = wc_pbp_check_active_addon($plugin_slug);
		$extraClass = '';
		$activate_button_text = __('Activate',PLUGIN_TXT);
		$inactivate_button_text = __('Deactivate',PLUGIN_TXT);
	...
	public function get_plugin_data( $plugin_file, $markup = true, $translate = true ) {
		...
		$plugin_data = get_file_data( $plugin_file, $default_headers, 'wc_pbp_plugin' );
	...
	public function get_addon_icon($data,$echo = true){
		$icon = PLUGIN_IMG.'addon_icon.jpg';
		if(file_exists($data['addon_root'].'icon.png') ){
			$icon = PLUGIN_ADDON_URL.$data['addon_folder'].'icon.png'; 
		} else if(file_exists($data['addon_root'].'icon.jpg')){
			$icon = PLUGIN_ADDON_URL.$data['addon_folder'].'icon.jpg';
		} else if(file_exists($data['addon_root'].$data['addon_slug'].'-icon.png')){
			$icon = PLUGIN_ADDON_URL.$data['addon_folder'].$data['addon_slug'].'-icon.png';
		} else if(file_exists($data['addon_root'].$data['addon_slug'].'-icon.jpg')){
			$icon = PLUGIN_ADDON_URL.$data['addon_folder'].$data['addon_slug'].'-icon.jpg';
